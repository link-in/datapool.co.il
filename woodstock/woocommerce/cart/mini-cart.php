		...
		<?php
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-mini-cart-item bag-product clearfix product-id-<?php echo esc_attr($cart_item['product_id']); ?><?php echo esc_attr($variation_id_class); ?> <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">                        
                        <td class="product-thumbnail"><a href="<?php echo get_permalink( $product_id ); ?>"><?php echo wp_kses_post($thumbnail); ?></a></td> 
                         
                        <td class="product-name">                        
                            <h4><a href="<?php echo get_permalink( $product_id ); ?>"><?php echo esc_attr($product_name); ?></a></h4>                    
                            <?php echo wc_get_formatted_cart_item_data( $cart_item ); ?>   
                            <?php echo apply_filters( 'woocommerce_widget_cart_item_quantity', '<span class="quantity">' . sprintf( '%s &times; %s', $cart_item['quantity'], $product_price ) . '</span>', $cart_item, $cart_item_key ); ?>
                        </td>
                        
                        <td class="product-remove">
							<a href="%s" class="remove-product remove" data-ajaxurl="<?php echo admin_url( 'admin-ajax.php' ); ?>" data-product-id="<?php echo esc_attr($cart_item['product_id']);?>"   data-variation-id="<?php echo esc_attr($cart_item['variation_id']);?>" data-product-qty="<?php echo esc_attr($cart_item['quantity']);?>" title="<?php echo esc_html__( 'Remove this item', 'woocommerce' ); ?>"><i class="icon-close-regular"></i></a>
                        </td>
                    </tr>
