...
<ul class="shop_table cart wishlist_table wishlist_view responsive mobile <?php echo esc_attr($show_cb) ? 'with-checkbox' : '' ?>" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr($wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token) ?>">
    ...
    if( $wishlist && $wishlist->has_items() ) :
                            ...
                            <div class="product-thumbnail-inner">
                                <?php if( $show_remove_product ): ?>
                                    <div class="product-remove">
                                        <div>
                                            <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ) ?>" class="remove_from_wishlist" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'woodstock' ) ); ?>">×</a>
                                        </div>
                                    </div>
                                <?php endif; ?>
                            ...
                            <?php if( $show_variation || $show_dateadded || $show_price ): ?>
                                <table class="item-details-table">
                                    ...
                                    <?php if( $show_dateadded && $item->get_date_added() ): ?>
                                        <tr>
                                            <td class="label">
                                                <?php _e( 'Added on:', 'woodstock' ) ?>
                                            </td>
                                            <td class="value">
                                                <?php echo wp_kses_post($item->get_date_added_formatted()); ?>
                                            </td>
                                        </tr>
                                    <?php endif; ?>
                            ...
                            <?php if( $show_quantity || $show_stock_status ): ?>
                                <table class="additional-info">
                                    ...
                                    <?php if( $show_stock_status ) : ?>
                                        <tr>
                                            <td class="label">
                                                <?php _e( 'Stock:', 'woodstock' ) ?>
                                            </td>
                                            <td class="value">
                                                <?php echo wp_kses_post($stock_status) == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . __( 'Out of stock', 'woodstock' ) . '</span>' : '<span class="wishlist-in-stock">' . __( 'In Stock', 'woodstock' ) . '</span>'; ?>
                                            </td>
                                        </tr>
                                    <?php endif ?>
                                </table>
                                ...
                                <div class="move-to-another-wishlist">
                                    <?php if( 'select' == $move_to_another_wishlist_type ): ?>
                                        <select class="change-wishlist selectBox">
                                            <option value=""><?php _e( 'Move', 'woodstock' ) ?></option>
                                            <?php
                                    ...
                                    <?php else: ?>
                                        <a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
                                            <?php echo apply_filters( 'yith_wcwl_move_to_another_list_label', __( 'Move to another list &rsaquo;', 'woodstock' ) ) ?>
                                        </a>
                                    <?php endif; ?>
                                </div>
                            <?php endif; ?>
        ...
        <p class="wishlist-empty">
            <?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'woodstock' ) ) ?>
        </p>
    <?php
    endif;
