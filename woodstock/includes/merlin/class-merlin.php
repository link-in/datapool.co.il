...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		add_action( 'woodstock_custom_fields_import', array( $this, 'import_custom_fields_and_taxonomies' ) );
	}
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'woodstock' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'woodstock' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'woodstock' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'woodstock' ),
		...
		$this->steps['info'] = [
			'name' => esc_html__( 'Server info', 'woodstock' ),
			'view' => [ $this, 'woodstock_info' ],
		];

		$this->steps['child'] = array(
			'name' => esc_html__( 'Child', 'woodstock' ),
			'view' => array( $this, 'child' ),
		);

		if ( $this->license_step_enabled ) {
			$this->steps['license'] = array(
				'name' => esc_html__( 'License', 'woodstock' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'woodstock' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'woodstock' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'woodstock' ),
	...
	protected function welcome() {
		...
		<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'woodstock' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'woodstock' ) );
	}
	...
	protected function woodstock_info() {
		...
		<div class="svg_holder">
			...
			<h1><?php esc_html_e( 'Server Requirements', 'woodstock' ); ?></h1>

			<p id="child-theme-text"><?php esc_html_e( 'Values are just a recommendation and not a strict requirement. But in the end, if the demo installation fails then please make sure that all values are met.', 'woodstock' ); ?></p>

			<?php if ( ! empty( $checks ) ) : ?>
				<div class="woodstock--step-info merlin__drawer--open">
					<div class="merlin__drawer merlin__drawer--install-plugins woodstock-info--content">
						<div class="woodstock-step--headers">
							<div class="header--first"></div>
							<div class="header--second"><?php esc_html_e( 'Suggested', 'woodstock' ); ?></div>
							<div class="header--third"><?php esc_html_e( 'Your Server', 'woodstock' ); ?></div>
						</div>
						<div class="woodstock-step--content">
					...
					<a class="merlin__button merlin__button--knockout merlin__button--no-chevron merlin__button--external"
					   href="<?php echo esc_url( 'https://temashdesign.ticksy.com/article/16767/' ); ?>"
					   target="_blank"><?php esc_html_e( 'Learn how to increase values', 'woodstock' ); ?></a>
				</div>
			<?php endif; ?>
		...
		<?php
		$this->logger->debug( __( 'The server info installation step has been displayed', 'woodstock' ) );
	}
	...
	protected function child() {
		...
		<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'woodstock' ) );
	}
	...
	protected function plugins() {
						...
						<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
							<li data-slug="<?php echo esc_attr( $slug ); ?>">
								...
								<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
									...
									<span class="badge">
									<span class="hint--top"
										  aria-label="<?php esc_html_e( 'Required', 'woodstock' ); ?>">
										<?php esc_html_e( 'req', 'woodstock' ); ?>
									</span>
								</span>
								</label>
							</li>
		...
		<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'woodstock' ) );
	}
	...
	protected function content() {
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'woodstock' ); ?>">
		...
		<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'woodstock' ) );
	}
	...
	protected function ready() {
		...
		do_action( 'woodstock_demo_completed' );
		?>
		...
		<?php
		$this->logger->debug( __( 'The final step has been displayed', 'woodstock' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( __( 'The existing child theme was activated', 'woodstock' ) );
		...
		$this->logger->debug( __( 'The newly generated child theme was activated', 'woodstock' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'woodstock' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'woodstock' ),
				)
			);
		}
		...
		$this->logger->debug( __( 'The license activation was performed with the following results', 'woodstock' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'woodstock' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						...
						/* translators: Expiration date */
							esc_html__( 'Your license key expired on %s.', 'woodstock' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'woodstock' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'woodstock' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'woodstock' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'woodstock' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'woodstock' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'woodstock' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'woodstock' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'woodstock' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'woodstock' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'woodstock' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'woodstock' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'woodstock' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'woodstock' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'woodstock' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'woodstock' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'woodstock' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'woodstock' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'woodstock' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'woodstock' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'woodstock' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'woodstock' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'woodstock' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'woodstock' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'woodstock' ),
				'description'      => esc_html__( 'Demo content data.', 'woodstock' ),
				'pending'          => esc_html__( 'Pending', 'woodstock' ),
				'installing'       => esc_html__( 'Installing', 'woodstock' ),
				'success'          => esc_html__( 'Success', 'woodstock' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'woodstock' ),
				'description'      => esc_html__( 'Sample widgets data.', 'woodstock' ),
				'pending'          => esc_html__( 'Pending', 'woodstock' ),
				'installing'       => esc_html__( 'Installing', 'woodstock' ),
				'success'          => esc_html__( 'Success', 'woodstock' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'woodstock' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'woodstock' ),
				'pending'          => esc_html__( 'Pending', 'woodstock' ),
				'installing'       => esc_html__( 'Installing', 'woodstock' ),
				'success'          => esc_html__( 'Success', 'woodstock' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'woodstock' ),
				'description'      => esc_html__( 'Sample theme options data.', 'woodstock' ),
				'pending'          => esc_html__( 'Pending', 'woodstock' ),
				'installing'       => esc_html__( 'Installing', 'woodstock' ),
				'success'          => esc_html__( 'Success', 'woodstock' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'woodstock' ),
				'description'      => esc_html__( 'Redux framework options.', 'woodstock' ),
				'pending'          => esc_html__( 'Pending', 'woodstock' ),
				'installing'       => esc_html__( 'Installing', 'woodstock' ),
				'success'          => esc_html__( 'Success', 'woodstock' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'woodstock' ),
				'description'      => esc_html__( 'After import setup.', 'woodstock' ),
				'pending'          => esc_html__( 'Pending', 'woodstock' ),
				'installing'       => esc_html__( 'Installing', 'woodstock' ),
				'success'          => esc_html__( 'Success', 'woodstock' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'woodstock' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'woodstock' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( class_exists( 'WPCF7' ) ) {
			update_post_meta( $contact_page_id->ID, 'contact_form', '[contact-form-7 id="2244" title="Contact Form"]' );
		}
    ...
    function set_woocommerce_pages( $settings ) {
      ...
      'woocommerce_pages' => 
		...
		woodstock_update_listing_expiring_dates();
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'woodstock' ), $import_file );
			}
		}
